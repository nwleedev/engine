...
SCAN_ROOTS = ("plugin-sources", "packages", "plugins")


def _python_files() -> list[Path]:
    files: list[Path] = []
    for root_name in SCAN_ROOTS:
        files.extend(sorted((ROOT / root_name).rglob("*.py")))
    return files
...
def test_plugin_python_sources_parse_as_python39() -> None:
    files = _python_files()
    assert files
...
def test_plugin_python_sources_do_not_import_tomllib_directly() -> None:
    failures: list[str] = []
...
def test_plugin_python_sources_do_not_use_runtime_pep604_annotations() -> None:
    failures: list[str] = []
    for path in _python_files():
        module = ast.parse(path.read_text(encoding="utf-8"), filename=str(path), feature_version=(3, 9))

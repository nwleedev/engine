# tests/quality-guard/test_feedback_compressor.py
import sys
import importlib
...
def test_parse_rules_from_result_invalid_json_returns_none():
    assert fc.parse_rules_from_result("not json") is None


def test_parse_rules_from_result_missing_rules_key_returns_none():
    assert fc.parse_rules_from_result('{"other": []}') is None


def test_write_rules_md_creates_file(tmp_path):
    fc.write_rules_md(str(tmp_path), ["rule one [2026-04-22]", "rule two [2026-04-22]"], source_count=2)
...
def test_write_rules_md_overwrites_existing(tmp_path):
    ...
    fc.write_rules_md(str(tmp_path), ["new rule"], source_count=1)
...
def test_run_compression_calls_claude_p_and_updates_files(tmp_path):
    ...
    with patch("subprocess.run", return_value=mock_result):
        fc.run_compression(str(tmp_path))

...
import pre_tool_use_handler as pth
...
def test_parse_verdict_superficial_high():
    ...
    verdict, reason, confidence = pth.parse_verdict(response)
...
def test_parse_verdict_unclear():
    ...
    verdict, _, confidence = pth.parse_verdict(response)
...
def test_parse_verdict_missing_fields_returns_defaults():
    verdict, reason, confidence = pth.parse_verdict("")
    assert verdict == "unclear"
    assert reason == ""
    assert confidence == "low"
...
def test_should_block_superficial_high_returns_true():
    assert pth.should_block("superficial", "high") is True

def test_should_block_superficial_medium_returns_false():
    assert pth.should_block("superficial", "medium") is False

def test_should_block_superficial_low_returns_false():
    assert pth.should_block("superficial", "low") is False

def test_should_block_structural_high_returns_false():
    assert pth.should_block("structural", "high") is False

def test_should_block_unclear_high_returns_false():
    assert pth.should_block("unclear", "high") is False
...
def test_non_dict_payload_produces_no_output():
    f = io.StringIO()
    with redirect_stdout(f):
        pth.main_with_payload("not a dict")
    assert f.getvalue() == ""

def test_unknown_tool_name_produces_no_output():
    f = io.StringIO()
    with redirect_stdout(f):
        pth.main_with_payload({"tool_name": "Bash", "tool_input": {"command": "ls"}})
    assert f.getvalue() == ""

def test_missing_tool_input_produces_no_output():
    f = io.StringIO()
    with redirect_stdout(f):
        pth.main_with_payload({"tool_name": "Edit"})
    assert f.getvalue() == ""

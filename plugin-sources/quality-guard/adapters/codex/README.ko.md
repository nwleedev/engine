...
`quality-guard`는 작업 턴이 superficial한 방식으로 끝났는지 점검하는 Codex 플러그인이다.

Codex 훅을 사용하지 않는다. 대신 재사용 가능한 스킬과 AGENTS.md 지침을 제공한다.

- `quality-guard:retrospect`는 현재 턴의 superficial 작업 여부를 회고한다.
- `quality-guard:install`은 AGENTS.md에 추천 회고 지침이 있는지 검사한다.

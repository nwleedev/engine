...
- do not write `role` or `parent_session_id` as flat `INDEX.md` source fields; parent-child relationships come from the Codex graph, and any printed relationship diagnostics are not INDEX frontmatter;
- for child sessions, do not create new `_children` paths or parent child entries;
...
- Required context headings are `current_goal`, `executive_summary`, `detailed_state`, `decisions`, `files`, `verification`, `risks`, `next_actions`, and `graph_context`.

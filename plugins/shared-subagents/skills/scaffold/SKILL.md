...
```bash
python3 plugins/shared-subagents/scripts/install_shared_subagents.py --dry-run
python3 plugins/shared-subagents/scripts/print_agents_md_block.py
```

To install into the real Codex home:

```bash
python3 plugins/shared-subagents/scripts/install_shared_subagents.py
```

To verify safely before touching the real Codex home:

```bash
python3 plugins/shared-subagents/scripts/install_shared_subagents.py --codex-home temps/2026-05-05/shared-subagents/codex-home
...
If installing into the real `~/.codex/agents/` directory, warn that existing files with the same names may be overwritten by the current installer.

...
Claude Code에서 구조화된 리서치 프로토콜을 강제합니다. 프롬프트 앞에 `/q`, `/query`, `/research`를 붙이면 Claude는 답변하기 전에 5단계 프로세스를 실행합니다: 가설 → 출처 검증 → 반론 확인 → 근본 원인 분석 → 최종 답변.

## 작동 방식

better-research는 하나의 훅을 등록합니다:

| 훅 | 실행 시점 | 동작 |
|---|---|---|
| `UserPromptSubmit` | Claude가 메시지를 처리하기 전 | 리서치 마커를 감지하고 프로토콜과 설정된 관점을 주입 |
...
마커는 Claude가 질문을 보기 전에 제거됩니다. Claude는 다음 단계를 실행합니다:

1. **초기 가설** — 초안 답변, 결론이 아님
2. **출처 검증** — 2개 이상의 독립적인 출처 (공식 문서, 명세서, 소스 코드 우선); 출처가 2개 미만인 주장은 `[UNVERIFIED]` 표시
3. **반론 확인** — 답변이 성립하지 않는 최소 1가지 제한 조건 또는 상황
4. **근본 원인 분석** — 재귀적 "왜" 체인 (최소 3단계); 근본 원인이 파악되기 전까지 해결책 제시 금지
5. **최종 답변** — 결론 → 근거 → 제한 사항 → 출처
...
마커 없이는 Claude가 평소대로 답변합니다. 마커를 붙이면 Claude는 5단계를 모두 거친 후 최종 답변을 제시합니다.

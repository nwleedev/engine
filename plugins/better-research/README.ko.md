...
직전 응답에서 텍스트 축약 패턴을 찾아 재작성합니다.

```
/antivague
/antivague "구체적 사례 없이 '여러 방법이 있다'고만 언급한 단락"
```

- 인자 없음: 직전 응답 전체를 재검토 대상으로 함
- 인자 있음: 해당 텍스트만 재검토

탐지 패턴:
- 막연한 열거: 구체적 내용 없이 "여러 방법이 있다"
- 예시 없는 주장: 원칙만 언급, 코드·수치 없음
- 단계 누락: 절차 설명 중 중간 단계 생략
- 단편적 문장: 근거 없는 결론 제시

재작성 후 패턴이 `.claude/feedback/raw.md`에 기록되어 다음 세션에 반영됩니다.

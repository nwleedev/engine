...
도메인 조사는 공식 문서에서 권장하는 패턴을 정리하는 것이 기본이다. 다음을 조사한다:

1. 해당 도메인/프레임워크/라이브러리의 공식 권장 패턴
2. 공식 문서가 명시하는 기본값과 권장 설정
3. 공식 문서가 명시하는 안티패턴
4. 해당 프레임워크 버전의 migration guide 변경 사항 (해당 시)

현재 프로젝트 코드 분석은 공식 기준과 다른 부분을 파악하기 위해 보충적으로 수행한다.
코드에서 발견한 패턴을 규칙의 근거로 사용하지 않는다.
...
Step 8에서 정리한 공식 문서 기반 규칙이 하네스의 기본 내용이다.
이 단계에서는 현재 프로젝트가 공식 기준과 다른 부분이 있는지 보충적으로 확인한다.
차이가 있으면 하네스에 별도 보충 항목으로 기록한다.
...
1. 공식 문서 권장 패턴 목록 대비 현재 프로젝트를 확인한다.
...
2. 차이가 있는 항목을 분류한다.
   - 의도적 선택: 프로젝트가 사유를 가지고 다르게 구현한 경우 — 사유와 함께 보충 기록
   - 미충족: 아직 적용하지 않은 경우 — 보충 기록
   - 해당없음 (not applicable): 이 프로젝트에 적용되지 않는 패턴

3. 공식 기준과 일치하는 항목은 별도 기록 불필요 (하네스 규칙이 이미 TO-BE 기반이므로).

4. 보충 분석 결과를 산출물에 반영한다.
   - 핵심 규칙: 공식 기준 대비 현재 프로젝트 상태 요약 + 프로젝트 보충 항목
   - 안티패턴: 미충족 격차 중 안티패턴 후보 + 프로젝트 보충 Anti/Good 쌍
   - 검증 기준: 패턴 충족 여부를 검증 기준에 포함 + 프로젝트 보충 체크리스트

#### 규칙

- 하네스 규칙의 근거는 공식 문서다. 현재 코드가 그 패턴을 따르고 있더라도 근거는 "공식 문서 권장"이다.
- 현재 프로젝트에만 존재하는 패턴(로컬 규약, 프로젝트 전용 헬퍼)은 하네스의 기본 규칙이 아니라 보충 항목으로 배치한다.
- 프로젝트의 현재 구조를 무비판적으로 수용하지 않는다. 평가 없이 현 상태를 곧 권장으로 서술하는 것은 금지한다.
- 프로젝트가 의도적으로 권장 패턴에서 일탈한 경우, 결함이 아닌 의식적 트레이드오프로 보충 기록한다.
...
- 코드 예시는 두 가지로 분류한다:
  - TO-BE 예시: 공식 문서/가이드에서 제공하는 코드 패턴. 하네스의 기본 예시.
  - 프로젝트 보충 예시: 현재 애플리케이션 구현에서 가져온 코드. 필요 시 별도 보충으로 추가.
- Good 예시는 공식 문서/가이드의 코드 패턴을 기본으로 작성한다.
- Anti 예시는 공식 문서가 명시하는 안티패턴을 기본으로 작성한다.

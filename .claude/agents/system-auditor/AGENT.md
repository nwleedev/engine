...
- **engine 레포**: `harness-*.md` 파일이 없는 것은 정상 (대상 프로젝트에서 harness-engine으로 생성됨). 코어 스킬(core-rules, failure-response, deep-study, research-methodology, socratic-thinking)과 harness-engine 디렉터리 구조만 검증.
...
1. **sync.sh 제외 목록**: 배포 전용 스크립트(bootstrap.sh, sync.sh, promote.sh, migrate.sh, test-sync.sh, install-hooks.sh)가 제외 목록에 포함되어 있는가
2. **install-hooks.sh 마커 정합성**: jq 필터의 마커 패턴(`ENGINE_HOOK=1`, `[engine-hook]`)이 실제 hooks/*.json의 마커와 일치하는가
3. **hooks/*.json 키 충돌**: 여러 hook 소스 파일이 같은 이벤트 키를 가지면 jq merge 시 덮어쓰기 발생 — 키 중복 검사

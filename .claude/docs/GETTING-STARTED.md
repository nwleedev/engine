...
```
my-project/
├── CLAUDE.md                        # ← 프로젝트별 규칙 작성
├── claude-scripts/                  # 자동 훅 스크립트 (수정 불필요)
│   ├── check-plan.sh
│   ├── check-plan-review.sh
│   ├── suggest-harness.sh
│   ├── track-edits.sh
│   └── snapshot.sh
├── suggest-harness-patterns.json    # ← 프로젝트별 패턴 커스터마이징 가능
├── .claude/
...
2. `suggest-harness-patterns.json` — 도메인 감지 패턴
3. `.claude/settings.local.json` — 권한 설정
...
```bash
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category dev-frontend
claude mcp add context7 -- npx -y @upstash/context7-mcp
claude mcp add playwright -- npx -y @playwright/mcp@latest
...
```bash
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category dev-backend-python
claude mcp add context7 -- npx -y @upstash/context7-mcp
npx lefthook install
...
git init
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category non-dev-research
claude mcp add tavily-mcp -- npx -y tavily-mcp
...
```bash
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category non-dev-marketing
claude mcp add tavily-mcp -- npx -y tavily-mcp
...
```bash
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category non-dev-design-doc
...
```bash
~/.claude-harness/claude-scripts/bootstrap.sh --target . --category non-dev-learning
...
### 5.1 `suggest-harness-patterns.json`

이 파일이 하네스 자동 제안의 핵심이다. 프로젝트에 맞게 수정한다.

**개발 프로젝트 예시 (Python):**
```json
{
  "file_glob": "^.*/src/.*\\.py$",
  "test_regex": "(test_.*|.*_test)\\.py$",
  "test_harness": "harness-be-testing",
  "patterns": [
    {
      "regex": "FastAPI|APIRouter|Depends|HTTPException",
      "harness": "harness-be-fastapi",
      "label": "FastAPI"
    },
    {
      "regex": "Session|Base\\.metadata|Column|relationship",
      "harness": "harness-be-sqlalchemy",
      "label": "SQLAlchemy"
    }
  ]
}
...
```json
{
  ...
  "patterns": [
    {
      "regex": "TAM|SAM|SOM|market size",
      "harness": "harness-research-market-sizing",
      "label": "Market Sizing"
    },
    {
      "regex": "competitor|SWOT|competitive",
      "harness": "harness-research-competitor",
...
| 하네스 제안이 안 나옴 | `suggest-harness-patterns.json` 없음/패턴 미매칭 | 파일 존재 확인 + 패턴 regex 테스트 |

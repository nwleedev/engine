...
# 설치 (한 줄, git 불필요)
sh -c "$(curl -fsSL https://raw.githubusercontent.com/nwleedev/claude-harness/main/install.sh)" -- .
```

업데이트도 같은 명령을 재실행하면 됩니다.

> 프로젝트 유형별 하네스는 설치 후 `/harness-engine`을 호출하면 자동 생성됩니다.
...
└── .claude/
    ...
    │   ├── check-plan.sh              # hook: 플랜 없이 편집 차단
...
프로젝트에서 만든 유용한 파일을 다른 프로젝트에서도 사용하고 싶다면, 하네스 저장소를 clone하여 `promote.sh`를 실행합니다:

```bash
git clone https://github.com/nwleedev/claude-harness.git /tmp/claude-harness

# 도메인 하네스 → 카테고리 예제로 승격
/tmp/claude-harness/.claude/scripts/promote.sh \
  --source ~/my-api \
...
```bash
# 각 프로젝트에 최신 하네스 동기화
sh -c "$(curl -fsSL https://raw.githubusercontent.com/nwleedev/claude-harness/main/install.sh)" -- ~/projects/my-api
sh -c "$(curl -fsSL https://raw.githubusercontent.com/nwleedev/claude-harness/main/install.sh)" -- ~/research/my-analysis
```
...
**Q: 팀원과 환경을 공유하려면?**
A: 팀원이 같은 `install.sh` 원라이너를 실행하면 됩니다. 프로젝트별 `.claude/` 디렉터리는 프로젝트 저장소에 커밋하여 공유합니다.

**Q: 프로젝트에서 만든 하네스를 다른 프로젝트에서도 쓰고 싶으면?**
A: 하네스 저장소의 `promote.sh`로 코어에 승격하세요. 도메인 하네스는 자동으로 카테고리 예제로 분류됩니다.
